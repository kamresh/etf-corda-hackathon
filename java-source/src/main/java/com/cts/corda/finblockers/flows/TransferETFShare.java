...
import com.cts.corda.finblockers.flows.ETFShareBaseFlow.SignTxFlowNoChecking;
...
import static com.cts.corda.finblockers.ETFCreationContract.OBLIGATION_CONTRACT_ID;

public class TransferETFShare {

    @StartableByRPC
    @InitiatingFlow
    public static class Initiator extends ETFShareBaseFlow {
        ...
        public SignedTransaction call() throws FlowException {
            ...
            final StateAndRef<ETFShareState> obligationToTransfer = getObligationByLinearId(linearId);
            final ETFShareState inputObligation = obligationToTransfer.getState().getData();
            ...
            if (!getOurIdentity().equals(lenderIdentity)) {
                throw new IllegalStateException("ETFShareState transfer can only be initiated by the lender.");
            }

            // Stage 4. Create the new obligation state reflecting a new lender.
            progressTracker.setCurrentStep(BUILDING);
            final ETFShareState transferredObligation = createOutputObligation(inputObligation);
            ...
            final Command transferCommand = new Command<>(new ETFCreationContract.Commands.Transfer(), signerKeys);
        ...
        @Suspendable
        private AbstractParty getLenderIdentity(ETFShareState inputObligation) {
        ...
        @Suspendable
        private ETFShareState createOutputObligation(ETFShareState inputObligation) throws FlowException {
            if (anonymous) {
        ...
        @Suspendable
        private Party getBorrowerIdentity(ETFShareState inputObligation) {
