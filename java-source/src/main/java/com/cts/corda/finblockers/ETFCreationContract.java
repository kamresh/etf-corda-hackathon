...
public class ETFCreationContract implements Contract {
    public static final String OBLIGATION_CONTRACT_ID = "com.cts.corda.finblockers.ETFCreationContract";
    ...
    private Set<PublicKey> keysFromParticipants(ETFShareState obligation) {
        return obligation
    ...
    private void verifyIssue(LedgerTransaction tx, Set<PublicKey> signers) {
        requireThat(req -> {
            ...
            ETFShareState obligation = (ETFShareState) tx.getOutputStates().get(0);
    ...
    private void verifyTransfer(LedgerTransaction tx, Set<PublicKey> signers) {
        requireThat(req -> {
            ...
            ETFShareState input = tx.inputsOfType(ETFShareState.class).get(0);
            ETFShareState output = tx.outputsOfType(ETFShareState.class).get(0);
    ...
    private void verifySettle(LedgerTransaction tx, Set<PublicKey> signers) {
        requireThat(req -> {
            ...
            List<ETFShareState> obligationInputs = tx.inputsOfType(ETFShareState.class);
            ...
            ETFShareState inputObligation = obligationInputs.get(0);
            ...
            List<ETFShareState> obligationOutputs = tx.outputsOfType(ETFShareState.class);
            ...
            if (amountOutstanding.equals(sumAcceptableCash)) {
                ...
                ETFShareState outputObligation = obligationOutputs.get(0);
