 ...
 */
abstract class ETFShareBaseFlow extends FlowLogic<SignedTransaction> {
    ...
    StateAndRef<ETFShareState> getObligationByLinearId(UniqueIdentifier linearId) throws FlowException {
        ...
        List<StateAndRef<ETFShareState>> obligations = getServiceHub().getVaultService().queryBy(ETFShareState.class, queryCriteria).getStates();
        if (obligations.size() != 1) {
            throw new FlowException(String.format("ETFShareState with id %s not found.", linearId));
        }
