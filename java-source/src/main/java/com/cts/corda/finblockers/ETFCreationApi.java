...
import com.cts.corda.finblockers.flows.ETFShareObligation;
import com.cts.corda.finblockers.flows.SettleETFShare;
import com.cts.corda.finblockers.flows.TransferETFShare;
...
@Path("obligation")
public class ETFCreationApi {
    private final CordaRPCOps rpcOps;
    private final Party myIdentity;

    public ETFCreationApi(CordaRPCOps rpcOps) {
        this.rpcOps = rpcOps;
    ...
    public Map<Currency, Long> owedPerCurrency() {
        return rpcOps.vaultQuery(ETFShareState.class).getStates()
                .stream()
    ...
    @Path("obligations")
    @Produces(MediaType.APPLICATION_JSON)
    public List<StateAndRef<ETFShareState>> obligations() {
        return rpcOps.vaultQuery(ETFShareState.class).getStates();
    }
    ...
    public Response issueObligation(
        ...
        // 3. Start the ETFShareObligation flow. We block and wait for the flow to return.
        try {
            final FlowHandle<SignedTransaction> flowHandle = rpcOps.startFlowDynamic(
                    ETFShareObligation.Initiator.class,
    ...
    public Response transferObligation(
        ...
        try {
            final FlowHandle flowHandle = rpcOps.startFlowDynamic(
                    TransferETFShare.Initiator.class,
                    linearId, newLender, true);

            flowHandle.getReturnValue().get();
            final String msg = String.format("ETFShareState %s transferred to %s.", id, party);
            return Response.status(CREATED).entity(msg).build();
        } catch (Exception e) {
            return Response.status(BAD_REQUEST).entity(e.getMessage()).build();
        }
    ...
    public Response settleObligation(
        ...
        try {
            final FlowHandle flowHandle = rpcOps.startFlowDynamic(
                    SettleETFShare.Initiator.class,
    ...
    @GET
    @Path("request-etfshare")
    public Response requestETFShares(
            @QueryParam(value = "amount") int amount,
            @QueryParam(value = "currency") String currency,
            @QueryParam(value = "party") String party) {
