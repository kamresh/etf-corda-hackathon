...
class ETFShareStateContractTransferTests : ETFShareStateContractUnitTests() {
    ...
    fun `must handle multiple command values`() {
        ledger {
            ...
            transaction {
                ...
                command(ALICE_PUBKEY, BOB_PUBKEY, CHARLIE_PUBKEY) { ETFCreationContract.Commands.Transfer() }
                this.verifies()
            }
        }
    }
    ...
    fun `only the lender may change`() {
        ledger {
            transaction {
                input(OBLIGATION_CONTRACT_ID) { ETFShareState(10.DOLLARS, ALICE, BOB) }
                output(OBLIGATION_CONTRACT_ID) { ETFShareState(1.DOLLARS, ALICE, BOB) }
                command(ALICE_PUBKEY, BOB_PUBKEY, CHARLIE_PUBKEY) { ETFCreationContract.Commands.Transfer() }
                this `fails with` "Only the lender property may change."
            }
            transaction {
                input(OBLIGATION_CONTRACT_ID) { ETFShareState(10.DOLLARS, ALICE, BOB) }
                output(OBLIGATION_CONTRACT_ID) { ETFShareState(10.DOLLARS, ALICE, CHARLIE) }
                command(ALICE_PUBKEY, BOB_PUBKEY, CHARLIE_PUBKEY) { ETFCreationContract.Commands.Transfer() }
                this `fails with` "Only the lender property may change."
            }
            transaction {
                input(OBLIGATION_CONTRACT_ID) { ETFShareState(10.DOLLARS, ALICE, BOB, 5.DOLLARS) }
                output(OBLIGATION_CONTRACT_ID) { ETFShareState(10.DOLLARS, ALICE, BOB, 10.DOLLARS) }
                command(ALICE_PUBKEY, BOB_PUBKEY, CHARLIE_PUBKEY) { ETFCreationContract.Commands.Transfer() }
                this `fails with` "Only the lender property may change."
            }
            transaction {
    ...
    fun `all participants must sign`() {
        ledger {
            transaction {
                input(OBLIGATION_CONTRACT_ID) { oneDollarObligation }
                output(OBLIGATION_CONTRACT_ID) { oneDollarObligation.withNewLender(CHARLIE) }
                command(ALICE_PUBKEY, BOB_PUBKEY) { ETFCreationContract.Commands.Transfer() }
                this `fails with` "The borrower, old lender and new lender only must sign an obligation transfer transaction"
            }
            transaction {
                input(OBLIGATION_CONTRACT_ID) { oneDollarObligation }
                output(OBLIGATION_CONTRACT_ID) { oneDollarObligation.withNewLender(CHARLIE) }
                command(ALICE_PUBKEY, CHARLIE_PUBKEY) { ETFCreationContract.Commands.Transfer() }
                this `fails with` "The borrower, old lender and new lender only must sign an obligation transfer transaction"
            }
            transaction {
                input(OBLIGATION_CONTRACT_ID) { oneDollarObligation }
                output(OBLIGATION_CONTRACT_ID) { oneDollarObligation.withNewLender(CHARLIE) }
                command(BOB_PUBKEY, CHARLIE_PUBKEY) { ETFCreationContract.Commands.Transfer() }
                this `fails with` "The borrower, old lender and new lender only must sign an obligation transfer transaction"
            }
            transaction {
                input(OBLIGATION_CONTRACT_ID) { oneDollarObligation }
                output(OBLIGATION_CONTRACT_ID) { oneDollarObligation.withNewLender(CHARLIE) }
                command(ALICE_PUBKEY, BOB_PUBKEY, MINI_CORP_PUBKEY) { ETFCreationContract.Commands.Transfer() }
                this `fails with` "The borrower, old lender and new lender only must sign an obligation transfer transaction"
            }
            transaction {
                input(OBLIGATION_CONTRACT_ID) { oneDollarObligation }
                output(OBLIGATION_CONTRACT_ID) { oneDollarObligation.withNewLender(CHARLIE) }
                command(ALICE_PUBKEY, BOB_PUBKEY, CHARLIE_PUBKEY, MINI_CORP_PUBKEY) { ETFCreationContract.Commands.Transfer() }
                this `fails with` "The borrower, old lender and new lender only must sign an obligation transfer transaction"
            }
            transaction {
                input(OBLIGATION_CONTRACT_ID) { oneDollarObligation }
