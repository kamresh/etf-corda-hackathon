...
class SettleETFShareTests : ETFShareStateTests() {
    ...
    fun `Fully settle non-anonymous obligation`() {
        ...
        network.waitQuiescent()
        assert(settleTransaction.tx.outputsOfType<ETFShareState>().isEmpty())
    ...
    fun `Partially settle non-anonymous obligation with non-anonymous cash payment`() {
        ...
        network.waitQuiescent()
        assert(settleTransaction.tx.outputsOfType<ETFShareState>().size == 1)
        ...
        val partiallySettledObligation = settleTransaction.tx.outputsOfType<ETFShareState>().single()
